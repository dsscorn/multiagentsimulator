...
public final class Evaluation {
     ...
     * @return 
     */
    public EvaluationResult evaluateDataSet(MultiLayerPerceptron multiLayerPerceptron, DataSet dataSet) {
        ...
        for (DataSetRow dataRow : dataSet.getRows()) {      // iterate all dataset rows
             // Sepide neuralNetwork.setInput(dataRow.getInput());    // apply input to neural network
			 multiLayerPerceptron.setInput(dataRow.getInput());
             multiLayerPerceptron.calculate();                     // and calculate neural network    
            ...
            for (Evaluator evaluator : evaluators.values()) { // for now we have only kfold and mse
                evaluator.processNetworkResult(multiLayerPerceptron.getOutput(), dataRow.getDesiredOutput());
            }
        }
    ...
    */
     ...
     */
    public static void runFullEvaluation(MultiLayerPerceptron multilayer, DataSet dataSet) {
        ...
        evaluation.evaluateDataSet(multilayer, dataSet);
