...
import org.apache.spark.api.java.JavaRDD;     //Sepide
import org.apache.spark.api.java.JavaSparkContext;    //Sepide
import org.apache.spark.broadcast.Broadcast;    //Sepide
import org.apache.spark.SparkConf;     //Sepide
...
{	
	...
	public static final int HIDDEN_NEURONS = 5;   // was 10 and changed to 5 on Jan. 10 2023 
...
//					myGui.appendResult(agentName+"\nTotal Tweets Processed: " + tweetCount + " TP:" + round(completionTimeTextProcessing/1000000.00,2) + "ms TFIDF:" + round(completionTimeTFIDF/1000000.00,2) + "ms K-means:" + round(completionTimeAlgorithm/1000000.00,2) + "ms Total:" + round((completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)/1000000.00,2)+"ms");
				...
				else if (algorithmRec == MLP)
				{
					...
					//Code added by Sepide for Cross-validation
					...
					// Create MultiLayerPerceptron for Cross-Validation with Weka 
                     
					//int seed = 1; // commented out on June 20-2023
                    //int folds = 10;   // commented out on June 20-2023
                    mlp = new MultilayerPerceptron();
					mlp.setLearningRate(LEARNING_RATE_MLP);
					mlp.setMomentum(0.2);
					mlp.setHiddenLayers("10");
					//Random random = new Random(seed); // commented out on June 20-2023
					
				try {    
				    //SparkConf conf = new SparkConf().setAppName("Distributed Training");  // Added July 21
					//JavaSparkContext sc = new JavaSparkContext(conf);   // Added July 21 
					//JavaRDD<String> trainingData = sc.textFile(trainSetFilePath);  // Added July 21
					//JavaRDD<String> testData = sc.textFile(testSetFilePath);   // Added July 21
					FileReader datareader = new FileReader(dataSetFilePath);
					FileReader trainreader = new FileReader(trainSetFilePath);
					FileReader testreader = new FileReader(testSetFilePath);
					...
					System.out.println("Test for Weka cross validation");
					System.out.println("\nFold " + (i+1) + ":\n" + eval.toSummaryString()); 
                    }  */  // Commented out on June 20-2023 
					...
					//Evaluation eval = new Evaluation(data); // Commented out on June 20-2023
					//eval.crossValidateModel(mlp, data, folds, new Random(seed));  // Commented out on June 20-2023
					//System.out.println("\n10-fold CV:\n" + eval.toSummaryString());  // Commented out on June-20-2023
					System.out.println("Mean Root Squared Error: "+eval.errorRate()); //Printing Training Mean root squared Error
					System.out.println("Mean Absolute Error: "+Double.toString(eval.meanAbsoluteError()));  // Mean Absolute Error
					System.out.println("Summary of Training" + eval.toSummaryString()); //Summary of Training
					System.out.println("Kappa statistic: "+Double.toString(eval.kappa()));  // Kappa statistic
					System.out.println("Estimated Accuracy: "+Double.toString(eval.pctCorrect()));
					System.out.println("Estimated Incorrectly classified: "+Double.toString(eval.pctIncorrect()));
					System.out.println("confusion matrix: "+ eval.toMatrixString());  // added on June 20-2023
					myGui.appendResult("Accuracy: " + eval.pctCorrect() + "%");
					//Sepide System.out.println("Precision: "+Double.toString(eval.precision(cls)));
					...
					// MLP - Neuroph NN
					/* commented out on Jan. 18 2023 MultiLayerPerceptron multiLayer = new MultiLayerPerceptron(TransferFunctionType.TANH, numUniqueDocTerms, HIDDEN_NEURONS, numFollowees);
					SoftMax softMaxAct = new SoftMax(multiLayer.getLayers().get(multiLayer.getLayers().size()-1));
					List<Layer> mlpLayers = multiLayer.getLayers();
					...
					for (Neuron outputNeuron : outputNeurons)
					{
					   ...
					   BackPropagation nodeLearningRule = (BackPropagation) multiLayer.getLearningRule();
					...
					//Create the class(followee) vectors  
					/*Comment-out by Sepide double[][] followeeClassVectors = new double[numFollowees][numFollowees];
					followeeNames = new String[numFollowees];
					// followeeNames = followeeFollowers.keySet().toArray(followeeNames);
					followeeNames = datasetFollowees.toArray(followeeNames);
					followeeIndex = new LinkedHashMap<String,Integer>();
					...
					//Jan. 18-2023 trainMLP = new DataSet(numUniqueDocTerms,numFollowees);			
					
					//Setup the vector data of each user for training
					/* Jan. 18 2023 for (String currUser : trainSetUsers)
					{
						Map<String,Double> currDocTfidf = allUserDocumentsTFIDF.get(currUser);
					...
					//Jan. 18 2023 testMLP = new DataSet(numUniqueDocTerms,numFollowees);
					
					//Setup the vector data of each user for test
					/* Jan. 18 2023 for (String currUser : testSetUsers)
					{
						Map<String,Double> currDocTfidf = allUserDocumentsTFIDF.get(currUser);
						double[] currUserTfidfVector = vectorArrayFormat(currDocTfidf,allUniqueDocTerms);
						int indexFollowee = followeeIndex.get(userFollowee.get(currUser));
					...
					//Jan. 18 2023 recMLP = new DataSet(numUniqueDocTerms,numFollowees);
					
					//Setup the vector data of each user for test
					/* Jan. 18 2023 for (String currUser : usersRec)
					{
						Map<String,Double> currDocTfidf = allUserDocumentsTFIDF.get(currUser);
						double[] currUserTfidfVector = vectorArrayFormat(currDocTfidf,allUniqueDocTerms);
						int indexFollowee = followeeIndex.get(userFollowee.get(currUser));
					...
					// create multi layer perceptron
					   /* MultilayerPerceptron mlpNode = new MultilayerPerceptron();  commented out on Jan.2 2023 
					   mlpNode.setLearningRate(LEARNING_RATE_MLP);
					   mlpNode.setMomentum(0.2);
					   mlpNode.setHiddenLayers("10");  */
					   
					/* Jan. 18 2023 nodeMLP = new MultiLayerPerceptron(TransferFunctionType.TANH, numUniqueDocTerms, HIDDEN_NEURONS, numFollowees);
					softMaxAct = new SoftMax(nodeMLP.getLayers().get(nodeMLP.getLayers().size()-1));
					mlpLayers = nodeMLP.getLayers();
					outputNeurons = mlpLayers.get(mlpLayers.size()-1).getNeurons();
					for (Neuron outputNeuron : outputNeurons)
					{
						outputNeuron.setTransferFunction(softMaxAct);
					}
					
					nodeLearningRule = (BackPropagation) nodeMLP.getLearningRule();
					nodeLearningRule.setLearningRate(LEARNING_RATE_MLP);
					nodeLearningRule.setMaxError(MAX_ERROR_MLP); 
					...
					//Jan. 11 2023 multiLayer.learn(trainMLP);
					...
					if (numRecAgents < 2)
					{
						...
						//Jan. 11 2023 testNeuralNetwork(multiLayer,testMLP);
						//Jan. 18 2023 testNeuralNetwork(nodeMLP,testMLP);
						//endTimeTest = System.nanoTime();
						//completionTimeTest = endTimeTest - startTimeTest;
						//Jan. 18 2023 recNeuralNetwork(nodeMLP,recMLP);
						//Jan. 11 2023 recNeuralNetwork(multiLayer,recMLP);
						//Jan.2 2023 MultilayerPerceptron mlp2 = new MultilayerPerceptron();
					    //Jan.2 2023 mlp2.setLearningRate(LEARNING_RATE_MLP);
					    //Jan.2 2023 mlp2.setMomentum(0.2);
					    //Jan.2 2023 mlp2.setHiddenLayers("10");
				...
				double clsLabel2 = 0.0;  // commented out on Jan.2 2023 
				...
				Instances data = null;
					...
					catch (IOException e)
					{
			         ...
			         for (int i = 0; i < recommendInstances2.numInstances(); i++) {
						...
						try {
							clsLabel2 = mlp.classifyInstance(recommendInstances2.instance(i));
						}
						catch (Exception e)
						{
							e.printStackTrace();
			   ...
			   for (String recUser : userPredictedFollowee2.keySet())
					{
						...
						endTimeAlgorithm = System.nanoTime();
						completionTimeAlgorithm = endTimeAlgorithm - startTimeAlgorithm;						
                        myGui.appendResult("Sepide:" + convertMs(completionTimeAlgorithm));
						//Output for MLP
	...
	//					myGui.appendResult(agentName+"\nTotal Tweets Processed: " + tweetCount + " TP:" + round(completionTimeTextProcessing/1000000.00,2) + "ms TFIDF:" + round(completionTimeTFIDF/1000000.00,2) + "ms K-means:" + round(completionTimeAlgorithm/1000000.00,2) + "ms Total:" + round((completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)/1000000.00,2)+"ms");
				  ...
				  if (algorithmRec == MLP && numRecAgents > 1)
				{
					...
					//Jan. 18-2023 String nodeMLPFileName = nnDirName+getLocalName()+"_MLP.nnet";
					//Jan. 18-2023 nodeMLP.save(nodeMLPFileName); 
					//Jan. 11 multiLayer.save(nodeMLPFileName);
					String nodeMLPFileName = nnDirName+getLocalName()+"_MLP.model";
					...
					//toAverageWeightsMsg.setContent("mlp.model");
				...
				/*catch(Exception e){
					...
					//BufferedReader recommendFile = readDataFile(recommendSetFilePath);
			        //Instances recommendInstances3 = null;
					
					/*try{
						
						recommendInstances3 = new Instances(recommendFile);
					}
					catch (IOException e)
					{
						e.printStackTrace();
				 ...
				 //recommendInstances3.setClassIndex(recommendInstances3.numAttributes() - 1);
			     //Instances predictedLabelsRec3 = new Instances(recommendInstances3);
			
				/*double clsLabel3 = 0.0;
				Map<String,String> userPredictedFollowee3 = new LinkedHashMap<String,String>();
				String predictedClass3;
				...
				Instances data = null;
					...
					catch (IOException e)
					{
			         ...
			         //for (int i = 0; i < recommendInstances3.numInstances(); i++) {
						...
						/*if (i == recommendInstances3.numInstances()-1)
						{
							predictedClass3 = predictedLabelsRec3.instance(i).stringValue(data.numAttributes()-1);
			//				System.out.println(predictedLabels.instance(i).stringValue(data.numAttributes()-2));
							System.out.println(getLocalName()+" Recommended Class: "+predictedLabelsRec3.instance(i).stringValue(data.numAttributes()-1));
							userPredictedFollowee3.put(usersRec.get(i),predictedClass3);
						}
			              }  */
			...
			/*for (String recUser : userPredictedFollowee3.keySet())
					{
						for (String followeeUser: followeeFollowers.keySet())
						{
							if (userPredictedFollowee3.get(recUser).equals(followeeUser))
								followeeScore = 1.0;
							else
								followeeScore = 0.0;
			  ...
			  if (msg!=null && msg.getOntology() == "Averaged MLP Complete")
			{
				...
				MultilayerPerceptron averagedMLP = new MultilayerPerceptron();
				...
				//Jan. 18 2023 averagedMLP = (MultiLayerPerceptron) NeuralNetwork.createFromFile(msg.getContent());
				
				startTimeAlgorithm = System.nanoTime();
				
				//Jan. 18-2023 startTimeTest = System.nanoTime();
				//Jan.18-2023 testNeuralNetwork(averagedMLP,testMLP);
				//MultilayerPerceptron mlpp = new MultilayerPerceptron();
				//System.out.println("test message by Sepide1");
				// testNeuralNetwork(averagedNN,testMLP);
				//Jan.18-2023 endTimeTest = System.nanoTime();
				//Jan.18-2023 completionTimeTest = endTimeTest - startTimeTest;
				//Jan.18-2023 recNeuralNetwork(averagedMLP,recMLP);
				...
				Instances data = null;
			        ...
			        for (int i = 0; i < recommendInstances.numInstances(); i++) {  //commented on Jan.2 2023 
...
//					System.out.println(agentName+"- Total Tweets Processed: " + tweetCount + " TP:" + convertMs(completionTimeTextProcessing) + "ms TFIDF:" + convertMs(completionTimeTFIDF) + "ms K-means:" + convertMs(completionTimeAlgorithm) + "ms Total:" + convertMs(completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)+"ms");
				System.out.println("Mapper"+nodeNumber+"- Total Tweets Processed: " + tweetCount + " TP: " + convertMs(completionTimeTextProcessing) + " ms TFIDF: " + convertMs(completionTimeTFIDF) + " ms Reducer"+ nodeNumber+ " MLP: " + convertMs(completionTimeAlgorithm) + " ms MLP Train: "+ convertMs(completionTimeTrain) + " ms MLP Test: "+ convertMs(completionTimeTest) +" ms Total: " + convertMs(completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)+" ms");
//					myGui.appendResult(agentName+"\nTotal Tweets Processed: " + tweetCount + " TP:" + round(completionTimeTextProcessing/1000000.00,2) + "ms TFIDF:" + round(completionTimeTFIDF/1000000.00,2) + "ms K-means:" + round(completionTimeAlgorithm/1000000.00,2) + "ms Total:" + round((completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)/1000000.00,2)+"ms");
				//Sepide myGui.appendResult("Mapper"+nodeNumber+"- Total Tweets Processed: " + tweetCount + " TP: " + convertMs(completionTimeTextProcessing) + " ms TFIDF: " + convertMs(completionTimeTFIDF) + " ms Reducer"+ nodeNumber+ " MLP: " + convertMs(completionTimeAlgorithm) + " ms MLP Train: "+ convertMs(completionTimeTrain) + " ms MLP Test: "+ convertMs(completionTimeTest) +" ms Total: " + convertMs(completionTimeTextProcessing+completionTimeTFIDF+completionTimeAlgorithm)+" ms");
