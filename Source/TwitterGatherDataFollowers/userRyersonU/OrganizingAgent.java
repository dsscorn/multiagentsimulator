...
//					}
			...
			{
				averageWeightCount++;
				MultilayerPerceptron receivedMLP = new MultilayerPerceptron();
				//Jan.18-2023 MultiLayerPerceptron receivedMLP = null;
				//Jan.18-2023 NeuralNetwork receivedNN = null;
				...
				//Jan.18-2023 receivedMLP = (MultiLayerPerceptron) NeuralNetwork.createFromFile(msg.getContent());
				//Jan.18-2023 listMLP.add(receivedMLP);
				...
				if (averageWeightCount == allRecAgents.length)
				{
					 ...
					 //Jan.18-2023 MultiLayerPerceptron averagedMLP = listMLP.get(0);
					...
					//Jan.18-2023 List<Layer> trainedLayers = listMLP.get(0).getLayers();
					// List<Layer> trainedLayers = listNN.get(0).getLayers();
					int trainedLayers = 3;
					String hL = averagedMLP.getHiddenLayers();
					System.out.println("Number of nodes in Hidden Layer: "+ hL);
					
					char firsthL = hL.charAt(0);
					int a = Integer.parseInt(String.valueOf(firsthL));
					
					/* commented out on Jan.18-2023 for (int i = 0 ; i < trainedLayers.size(); i++)
					{
					...
					String averagedMLPFileName = nnDirName+getLocalName()+"_MLP.model";
					//Jan.18-2023 String averagedMLPFileName = nnDirName+"averaged_MLP_weights.nnet";
					//Jan.18-2023 averagedMLP.save(averagedMLPFileName);
